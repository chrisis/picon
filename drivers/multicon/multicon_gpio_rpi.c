/*
 * NES, SNES and SEGA gamepad driver for Raspberry Pi
 *
...
/*
 * All valid GPIOs found on the Raspberry Pi Header.
 */
...
MODULE_DESCRIPTION("NES, SNES and SEGA gamepad driver for Raspberry Pi");
MODULE_LICENSE("GPL");
 ...
 * 
 * @param ptr The pointer to the multicon_config structure
 */
static void multicon_timer(unsigned long ptr) {
	struct multicon_config* cfg = (void *) ptr;
 ...
 * Enables the timer if at least one device is open.
 */
static int multicon_open(struct input_dev* dev) {
	struct multicon_config* cfg = input_get_drvdata(dev);
	int status;
	...
	if (cfg->driver_usage_cnt > 0) {
		// At least one device open. Start the timer or reset the timeout.
		mod_timer(&cfg->timer, jiffies + REFRESH_TIME);
	}
 ...
 * Disables the timer if the last device is closed.
 */
static void multicon_close(struct input_dev* dev) {
	struct multicon_config* cfg = input_get_drvdata(dev);
 ...
 */
module_param_array_named(gpio, multicon_config.gpio_id, uint, &(multicon_config.gpio_id_cnt), S_IRUGO);
 ...
 */
module_param_named(multitap, multicon_config.pads_cfg.multitap_enabled, bool, S_IRUGO | S_IWUSR);
...
static int __init multicon_init(void) {
	...
	for (i = 0; i < NUMBER_OF_GPIOS; ++i) {
		multicon_config.pads_cfg.gpio[i] = gpio_get_bit(multicon_config.gpio_id[i]);
	}
	...
	status = pads_setup(&multicon_config.pads_cfg);
	if (status != 0) {
	...
	mutex_init(&multicon_config.mutex);
	setup_timer(&multicon_config.timer, multicon_timer, (long) &multicon_config);
 ...
 */
static void __exit multicon_exit(void) {
	del_timer(&multicon_config.timer);
	pads_remove(&multicon_config.pads_cfg);
	mutex_destroy(&multicon_config.mutex);
	gpio_exit();

	pr_info("driver exit\n");
}

module_init (multicon_init);
module_exit (multicon_exit);